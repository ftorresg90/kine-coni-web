...
export default function BookingForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      window.open(`https://wa.me/56982927833?text=${waText}`, '_blank')
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit}
      ...
      <div>
        ...
        {errors.nombre && <p className="mt-1 text-xs text-red-500 font-sans">{errors.nombre}</p>}
      </div>
      ...
      <div>
        ...
        {errors.telefono && <p className="mt-1 text-xs text-red-500 font-sans">{errors.telefono}</p>}
      </div>

      <div>
        <label htmlFor="servicio" className="block font-sans text-sm font-bold text-vino mb-2">Tipo de servicio *</label>
        <select id="servicio" name="servicio" onChange={() => clearError('servicio')} disabled={success}
        ...
        {errors.servicio && <p className="mt-1 text-xs text-red-500 font-sans">{errors.servicio}</p>}
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          {errors.fecha && <p className="mt-1 text-xs text-red-500 font-sans">{errors.fecha}</p>}
        </div>
        <div>
